...
| TASK-0120 | Phase 12.5A | Place Intelligence プロバイダ選定・APIキー計画 | プロバイダ確定・Google Cloud 設定 | TASK-0090, TASK-0054 | P0 | プロバイダ決定・APIキー準備完了 |
| TASK-0121 | Phase 12.5B | PlaceCandidate / PlaceGroup データモデル実装 | Firestore スキーマ・型定義・Rules | TASK-0120, TASK-0012 | P0 | 型定義・Repository・Rules 追加完了 |
| TASK-0122 | Phase 12.5C | Place enrichment Cloud Functions 実装・デプロイ | `enrichNotePlaces` / `getPlaceCandidatesForGroup` | TASK-0121, TASK-0054 | P0 | Functions デプロイ完了 |
| TASK-0123 | Phase 12.5A | Places API secret / キー管理設定 | Secret Manager 登録・キー制限設定 | TASK-0120 | P0 | Secret 登録・Cloud Console 制限設定完了 |
| TASK-0124 | Phase 12.5C | 候補取得実装（Google Places Nearby Search） | API呼び出し・レスポンス整形・Firestore 保存 | TASK-0122, TASK-0123 | P0 | 候補取得・キャッシュ動作確認済み |
| TASK-0125 | Phase 12.5D | 候補スコアリング・AI ランキング実装 | スコアリング関数・AI プロンプト・候補自動選択 | TASK-0124, TASK-0055 | P0 | confidence スコア出力確認済み |
| TASK-0126 | Phase 12.5E | ユーザー確認 / 手動修正 UI 実装 | places・places/[id]・manual 画面 | TASK-0125, TASK-0064 | P0 | 候補選択・手動入力ができる |
| TASK-0127 | Phase 12.5E | Detail 画面への「訪れた場所」セクション追加 | VisitedPlacesSection コンポーネント | TASK-0126 | P0 | Detail に場所情報が表示される |
| TASK-0128 | Phase 12.5F | Map SDK 導入・ピン表示実装 | react-native-maps・EAS Build 対応 | TASK-0126, TASK-0048 | P1 | 本格地図でピン表示できる |
| TASK-0129 | Phase 12.5A | プライバシー・コスト・利用制限 設計確定 | コスト上限・Rate Limit・ログ方針 | TASK-0120 | P0 | ポリシー確定・Cloud Console 上限設定完了 |
| TASK-0130 | Phase 12.5E | AI 日記・共有カードへの場所情報連携 | generateMemoryDiary プロンプト更新・shareCard 場所表示 | TASK-0127, TASK-0057, TASK-0085 | P1 | AI 日記に場所名が反映される |
...
## 17. Phase 12.5: Place Intelligence / Location Enrichment

### 目的
写真 GPS から「どこに行ったか」を名称・カテゴリとして推定し、アプリの中核価値を完成させます。

### サブフェーズ
- **12.5A**: プロバイダ選定・APIキー準備・コストポリシー確定
- **12.5B**: PlaceGroup / PlaceCandidate データモデル・Firestore Rules
- **12.5C**: Cloud Functions 候補取得（enrichNotePlaces / getPlaceCandidatesForGroup）
- **12.5D**: スコアリング・AI ランキング
- **12.5E**: 場所確認 UI・Detail 統合・AI 日記/共有カード連携
- **12.5F**: Map SDK（react-native-maps）・EAS Build 対応

### 主要タスク
- `TASK-0120` プロバイダ選定
- `TASK-0121` データモデル
- `TASK-0122` Cloud Functions
- `TASK-0123` APIキー管理
- `TASK-0124` 候補取得
- `TASK-0125` スコアリング
- `TASK-0126` 確認UI
- `TASK-0127` Detail統合
- `TASK-0128` Map SDK
- `TASK-0129` プライバシー/コスト
- `TASK-0130` AI日記/共有カード連携

### 完了イメージ
- 写真から訪れた場所名が自動推定される
- ユーザーが場所を確認・修正できる
- 場所名が AI 日記と共有カードに反映される

**詳細:** `docs/phase12_5_place_intelligence/` を参照

---

## 18. Phase 13: Search / Calendar / Tags
