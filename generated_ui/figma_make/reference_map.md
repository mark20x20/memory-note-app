...
| SCR-NOTE-001 | ノート詳細 | `/(app)/notes/[noteId]` | `app/(app)/notes/[noteId]/index.tsx` | ✅ Phase 10 更新済み（nested route 移行・編集ボタン追加） | Phase 7〜10 | ○ | **A** | phase9_ai_generation/ | [noteId].tsx → [noteId]/index.tsx に移行 |
| SCR-NOTE-002 | ノート編集 | `/(app)/notes/[noteId]/edit` | `app/(app)/notes/[noteId]/edit.tsx` | ✅ Phase 10 実装済み（タイトル・メモ・種別・AI日記編集・削除） | Phase 10 | ○ | B | phase10_note_detail/ | |
...
| SCR-MAP-001 | ノート地図（地図風プレビュー） | `/(app)/notes/[noteId]` (Detail内) | `src/features/map/components/MapPreview.tsx` | ✅ Phase 8 実装済み（地図風プレビュー・ピン表示・PlaceGrouping。外部SDK不使用） | Phase 8 | ○ | **A** | phase8_map/ | 本格地図 SDK は Phase 10 以降 |
