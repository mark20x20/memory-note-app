...
| 画面ID | 仕様書の画面名 | 現在のルート / 予定ルート | 実装ファイル | 実装状態 | 実装Phase | Figma生成対象 | Figma作成優先度 | Figma草案保存場所 | 備考 |
|---|---|---|---|---|---|---|---|---|---|
...
| SCR-HOME-001 | ホーム / ノート一覧 | `/(app)/home` | `app/(app)/home.tsx` | 🟡 Placeholder | Phase 5〜 | ○ | **A** | phase5_memory_note_creation/ | ノートカードあり状態のUI |
...
| SCR-CREATE-002 | 写真選択 | `/(app)/create` | `app/(app)/create/index.tsx` | ❌ 未実装 | Phase 6 | ○ | **A** | phase5_memory_note_creation/ | expo-image-picker |
| SCR-UPLOAD-001 | アップロード進捗 | `/(app)/create/upload` | `app/(app)/create/upload.tsx` | ❌ 未実装 | Phase 7 | ○ | **A** | phase6_upload_processing/ | プログレスバー |
...
| SCR-NOTE-001 | ノート詳細 | `/(app)/notes/[noteId]` | `app/(app)/notes/[noteId]/index.tsx` | 🟡 Placeholder | Phase 9〜10 | ○ | **A** | phase9_ai_generation/ | 実データ取得は Phase 9 |
...
| SCR-ERR-003 | アップロード失敗 | — | `/(app)/create/upload` 内 | ❌ 未実装 | Phase 7 | ○ | **A** | phase6_upload_processing/ | 再試行UI |
...
Figma Make でUI草案を生成する際のガイドライン（詳細は各ドキュメント参照）:

1. **共通ベースプロンプト**: `generated_ui/figma_make/figma_make_common_prompt.md` §2 を使用
2. **画面別追加プロンプト**: `generated_ui/figma_make/figma_make_common_prompt.md` §3 を参照
3. **UIデザインルール**: `generated_ui/figma_make/ui_design_system.md` に従う
4. **生成・保存ルール**: `generated_ui/figma_make/generation_rules.md` に従う
5. **実装反映ルール**: `generated_ui/figma_make/implementation_rules.md` に従う
6. **Phase計画**: `generated_ui/figma_make/phase_plan.md` を参照
7. **画面優先順位**: `generated_ui/figma_make/screen_priority.md` を参照
